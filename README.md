...
| `GET /api/org/settings` \| `PATCH /api/org/settings` | Org-level settings -- currently the review-queue confidence threshold, a Business/Scale-only override (`customConfidenceThreshold` in `plans.js`) of the server-wide `REVIEW_CONFIDENCE_THRESHOLD` default. `PATCH` with `{confidence_threshold: null}` always resets to the default, on any plan |
