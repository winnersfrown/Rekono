...
- Confidence scoring combines per-field confidence with an automatic cross-check: do the line items sum to the subtotal, and does `subtotal + shipping + other charges - discount + tax` equal the stated total? A failed cross-check pulls overall confidence down independent of what the model claimed, and its message names what it counted so the disagreement is debuggable from the review queue rather than requiring the document.

- **Everything between the subtotal and the total is extracted**, not just tax: shipping and discount as their own fields, plus an open labelled list (`other_charges`) for handling, surcharges, deposits applied and whatever else a vendor prints. This is a correctness requirement, not a nicety -- until v1.39 the cross-check computed `subtotal + tax`, so every invoice with a shipping line failed against a document that added up perfectly. Payment terms (`2/10 n/30`, `Net 30`) are captured as printed and deliberately not parsed into a due date; inventing one from a misread term is how a bill gets paid late. Tax and discount percentages are derived from the stored figures for display and never stored themselves.
...
**Checks** (`extractionChecks.js`, `confidenceChecks.js`, `checkPipeline.js`, `routes/checks.js`, `Check` model): a sixth document-processing pipeline, same shape as the five above, applied to scanned or photographed checks -- payee, amount, date, check number, memo, bank, and the last four digits of the account number. What makes this one different from the other four "extra" pipelines is that a check is not filed, it is **applied**: `POST /api/checks/:id/link` records a real `BillPayment` and posts a real journal entry (Debit Accounts Payable / Credit whatever the money left from), and `POST /api/checks/:id/unlink` reverses it. That posting goes through `accountsPayable.js`'s `recordBillPayment` rather than a payment path of its own -- the same function the manual payments screen and the QuickBooks bank-match confirmation call, which already refuses to relieve a payable that never posted and already unwinds its own row when the ledger rejects the entry. A check arriving by camera rather than by keyboard is not a reason for money to reach the books down a second road that has to be kept in step by hand.

Three things are specific to this module. **First, the MICR line is narrowed to the last four digits of the account number and the routing number is not stored at all.** The strip along the bottom of every check carries both, and that pair is the whole of what someone needs to draft an ACH debit against the account -- keeping it would make this one table a more attractive target than the rest of the app combined, for no product benefit. The narrowing happens at the same three points the tax module narrows a TIN: extraction (applied to the LLM's output too), the correction route (the review field deliberately has *no* `maxlength`, since a 4-character cap keeps the *first* four digits), and the raw OCR text (`redactMicr` masks every run of 8+ digits before `rawOcrText` is persisted). **Second, the payment is dated from the check, not from the scan** -- a check written on the 28th and photographed on the 3rd belongs in the month it was written, and using the upload date moves real money across a period boundary silently. **Third, `GET /api/checks/:id/match-suggestions` scores the org's open bills against the check** using `matching.js`'s `scorePair` (exported for this, so the amount tolerance and date window stay one configuration rather than two that drift), against each bill's *outstanding balance* rather than its face value -- a $500 check against a $2,000 bill with $1,500 already paid is an exact match, and scoring it against the total ranks it below a coincidence. The check's memo line is passed as the reference, since that is where people write the invoice number they're paying. Fully paid bills drop out entirely rather than being offered and then refused as an overpayment.

Linking is the only route to `approved`, so the status can never claim a payment that isn't on the books, and a linked check can't be corrected, re-extracted, or deleted -- its fields are what the posted payment was based on. Unlinking reverses the entry (both halves stay on the books and cancel, same as every other correction in this ledger) and returns the check to `needs_review` rather than `extracted`: a link that had to be undone is evidence something was misread. Reachable from the dashboard's "Checks" sidebar tab, whose default view is the "Not yet applied" filter -- checks representing money that left the account with nothing on the books accounting for it. No CSV/Excel export yet, unlike the other five: a check's value is the link, and the payment it creates already exports through the AP reports.
...
| `GET /api/statements/profit-and-loss` | Multi-step income statement over `?from=`/`?to=` (defaults to year-to-date): revenue, cost of revenue, gross profit, operating expenses, operating income, tax, net income. Accrual basis |
...
| `GET /api/invoices/:id` | Full invoice detail incl. line items, confidence, match results, derived tax/discount rates, and payment status from the payments recorded against it |
...
`POST /api/demo/login` is a public, unauthenticated endpoint (rate-limited per IP, same shape as signup) that spins up a brand-new org, seeds it with a realistic, varied-status dataset across all five document pipelines (invoices, expense receipts, vendor documents, leases, tax documents) plus matching data, an audit trail, and a **fully worked general ledger** (`src/demoSeed.js`), and returns a working bearer token -- no signup, no email, no card. It's meant for investors/prospects to click straight into a populated instance: the marketing site's hero has a "View live demo" link (`?demo=1` on the app's own origin), which `public/auth.js` detects on load and exchanges for a session the same way the Google sign-in handoff works. A logged-in demo session shows a persistent "Demo Mode" banner in the app shell with a one-click path to the real signup form.

The ledger half of that seed is six months of trading posted through `postJournalEntry` -- the same single write path a real posting uses, so the demo's books are subject to exactly the balance checks a customer's are. Revenue grows month over month, cost of revenue tracks it at roughly 38% so gross margin is worth looking at, receivables are collected the following month so AR aging and the cash flow statement have something real, and an open close period with half its checklist ticked means the Close tab shows a close in progress rather than an empty state. Two gaps are deliberate: a month of rent that never posted and a fixed asset with nothing depreciating it, which are the two things `closeAutomation.js` looks for -- so the Close tab surfaces real suggestions instead of a clean bill of health that demonstrates nothing.
...
**Income statement** (also called the profit & loss; `?from=`/`?to=`, defaults to year-to-date) is accrual basis: an approved invoice hits it the moment it's approved, not when it's paid. It is presented multi-step -- revenue less cost of revenue is gross profit, less operating expenses is operating income, less tax is net income -- because gross profit says whether the thing being sold makes money and operating income says whether the company around it does, and a single-step statement can't tell those two failures apart. **Balance sheet** (`?as_of=`, defaults to today) is a point-in-time snapshot. **Cash flow** (`?from=`/`?to=`) is the direct method -- every entry that moved cash, classified by what the cash moved *against*: revenue/expense counter-accounts are operating, other assets are investing, equity and debt are financing.

Three design decisions worth knowing:

- **Cost of revenue is a subtype, not a type.** An account carrying subtype `cost_of_revenue` is still an expense in every other sense: debit-normal, closes into retained earnings, and the trial balance doesn't care. Only the income statement treats it differently, and it finds it by subtype rather than by name so renaming an account can't move a reported figure (same reasoning as the income tax line). An org with no such account has zero cost of revenue, so gross profit equals revenue, `expenses.total` is unchanged, and the statement collapses back to the single-step shape -- which is what makes the classification safe to add to books that already exist.
